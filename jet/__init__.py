...
class JET(BaseEstimator, ClusterMixin):
    def __init__(
        ...
        self._pre_clustering = PreClustering(
            n_clusters=self.n_pre_clusters, n_jobs=self.n_jobs, verbose=self.verbose
        )
        self._distance_matrix_estimator = DistanceMatrixEstimator(
            n_jobs=self.n_jobs, verbose=self.verbose, metric=self.metric, c=self.c
        )
        self._ward_clustering = WardClustering(
            n_clusters=self.n_clusters, n_jobs=self.n_jobs, verbose=self.verbose
        )

    def fit(self, X: List[np.ndarray], y: Optional[np.ndarray] = None) -> JET:
        if len(X) <= 150:
            warn(
                "The number of time series is less than 150. It's probably faster to use the exact algorithm."
            )

        encoded = self._feature_encoder.fit_transform(X)
        pre_labels = self._pre_clustering.fit_predict(encoded)
        estimated_distance_matrix = self._distance_matrix_estimator.fit_transform(
            X, pre_labels
        )
